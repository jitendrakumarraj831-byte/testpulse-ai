...
- [x] 4.4 – **Resource Control Center**: `/admin/resource-center` publishes three distinct catalogs — one at a time via a dynamic form, or in bulk via an Excel/CSV importer (`title`, `description`, `catalog_type`, `resource_url` columns, batch-inserted per target table with per-row validation results). **Study Resources** route into the existing `resources`/library table (book/notes/catalog). **Active Offers & Banners** are a new `promotional_offers` table — the currently-active one renders as a dismissible banner at the top of the public `/` homepage. **Institutional Documents** are a new `institutional_documents` table, visible to any signed-in user at `/student/documents`. Offers and documents are deliberately separate tables from the library rather than new `resources.category` values, so `library_catalog()`/`/library` keep their real, study-content-only semantics — see the schema.sql comment above `promotional_offers`.
