-- TestPulse AI core schema — PART 1 of 2.
-- This file is too large to paste into the Supabase SQL Editor in one go,
-- so the full schema.sql is split here at a clean statement boundary.
-- Run this file first, then run schema_part2.sql in the same project
-- (SQL Editor, one paste per file, or `supabase db push` against the
-- combined schema.sql).
